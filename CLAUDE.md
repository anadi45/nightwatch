...
- Light sources: cool moonlight directional from the moon position (8,13,-44) at intensity 1.0, ambient 0x2a3a5a @ 0.75, a dim forward fill directional (0x3a4d6a @ 0.45) so gameplay is readable, a warm lantern-pool PointLight near the player (2.2, range 20), and the pistol's teal energy/muzzle lights. The ground has a baked emissive moonlight pool.
...
- **`src/client/engine/`** — Game engine modules:
  ...
  - `Creature.ts` — the alien, **fully procedural, no model asset**: lathe-profile shroud body + alien-cranium lathe head (tiny chin flaring to a wide ovoid dome) + tail tendrils (no arms), all in one per-instance clone of a fresnel ShaderMaterial (additive, teal 0x00ddaa rim that crosses the bloom threshold at glancing angles, vertex-shader waver, manual fade-to-black fog matching World's density 0.06, `uDissolve` eats the body tail-up on death). Almond eyes: 3 meshes per eye (void-black shell + teal HDR iris + additive glow), all sphere geo flattened via mesh scale; iris/glow opacity pulses in `animate()` (eyeMats layout is [void, iris, glow] × 2 — indices matter). Wisp trail, additive aura, movement patterns fitted inside `X_BOUND` (weave recentered, hard clamp for all), state machine, hit sphere, `getHitCenter()`. Dies by dissolve + scale-collapse + teal fx burst + short-lived orange impact PointLight. `dispose()` disposes only owned materials — geometry is module-level shared statics.
  ...
  - `environment/Sky.ts` — 3-stop gradient dome (horizon 0x3d4a68 → mid 0x1a2440 → zenith 0x0a0e22, BackSide ShaderMaterial), ~200 small dim twinkling star Points (vertex-shader animated, cool #c8d8f4 tint), moon upper-right at (8,13,-44) with radial-gradient disc (#f2f6ff → #8ca8d0) + 4 layered additive halos, two hand-colored hill ridge layers (ShapeGeometry paper-cut silhouettes, farther = lighter). All `fog: false`.
