...
- Aliens self-glow **violet** (0xb069ff rim, fresnel shader, hue-jittered per instance) with a pulsing violet inner core — no eyes. The pistol/bolt keep **teal** (0x00ddaa family) so player tech and alien organisms never share a color. The world lantern pool is the only warm element.
...
- **Aliens** (floating octopus-like entity: bulbous breathing bell, seven writhing tentacles, violet fresnel rim, no eyes — the pulsing violet inner core is the visual anchor, trailing wisps) use movement patterns (straight/weave/zigzag/flank) that get trickier as time passes, always staying inside the fence line (`X_BOUND`)
