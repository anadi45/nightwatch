...
Every visual decision obeys one rule: **the scene is layered near-black paper-cut silhouettes against a luminous moonlit sky; the aliens' violet glow and the pistol's teal energy elements are the only bright things — two species of light, one per side.** Concretely:
...
- Environment objects (trees, boulders, fence, rocks, grass) use unlit near-black `MeshBasicMaterial` (`SILHOUETTE_MAT`, 0x05060c) — silhouettes don't take light. Everything is procedural; there are no model assets.
...
- Aliens self-glow **violet** (0xb069ff rim, fresnel shader, hue-jittered per instance) with a pulsing violet inner core — no eyes. The mothership (fog-free backdrop layer) shares that violet in its bay/lights/beam. The pistol/bolt keep **teal** (0x00ddaa family) so player tech and alien organisms never share a color. The world lantern pool is the only warm element.
...
- **Aliens** (floating octopus-like entity: bulbous breathing bell, seven writhing tentacles, violet fresnel rim, no eyes — the pulsing violet inner core is the visual anchor, trailing wisps) **drop out of the mothership's underbelly bay** (eased descent over ~0.9s from `SHIP_BAY_Y` via the `dropFrom` config) then use movement patterns (straight/weave/zigzag/flank) that get trickier as time passes, always staying inside the fence line (`X_BOUND`)
...
- Consecutive alien-reaches make newly spawned aliens approach faster — linear `+SPEED_PENALTY` per reach, hard-capped at `MAX_SPEED` (1.5× base); a hit resets the speed to base. (The penalty must never accumulate multiplicatively — an early version spiralled unwinnably.)
...
- **Two watches per day** per logged-in player, server-enforced (UTC day, counter key with 48h expiry). Starting a run consumes a play even if abandoned. Logged-out players are uncapped (casual, no persistence). **Playtest subreddits are uncapped** — `UNCAPPED_SUBREDDITS` in `routes/api.ts` (must match devvit.json's `dev.subreddit`) so devs can run unlimited watches; the counter still increments (it feeds the `runId`).
...
- **`src/client/engine/`** — Game engine modules:
  ...
  - `PostFX.ts` — EffectComposer + UnrealBloomPass + film ShaderPass (gentle vignette to ~72% corners, whisper-level dither grain (0.006 — anything stronger reads as TV static on the dark scene; it exists to break sky-gradient banding), slight cool grade — runs on the linear HDR buffer before OutputPass) + OutputPass. Selective bloom via HDR color boosting: emissive materials multiply their color above 1.0 and the bloom threshold is 1.0, so only eyes/energy-vents/muzzle/moon/alien-rims bloom. Custom HalfFloat MSAA render target (WebGL2); falls back to direct rendering on WebGL1 (`postfx.enabled === false`). ACES tone mapping is set on the renderer in World.
  - `Creature.ts` — the alien, **fully procedural, no model asset**: floating octopus — bulbous bell lathe with a 7-lobed scalloped lip (displaced post-lathe + `computeVertexNormals`; lip y 0.88, crown 1.63, breathing scale pulse + random y-spin per instance in `animate()`/constructor) + seven tapered-cylinder tentacles with a baked S-curve hanging from a ring inside the lip (length-jittered, static outward splay stored in `tentacleBaseRx/Rz` arrays and reapplied under the per-frame root sway) + violet HDR inner core sphere pulsing with the breath (no eyes — the core is the focal glow). The fragment shader adds pulsing bioluminescent freckles (sin-product dot lattice) and faint radial striations masked to the bell band. No aura shell — the rim/freckles do the glow. Two per-instance clones of the fresnel ShaderMaterial (**normal blending, opaque near-black body + violet 0xb069ff rim, hue-jittered ±0.025 per instance**; alpha 1.0 with `depthWrite: true` so the body self-occludes — `transparent` stays true only for dissolve/fade; manual fog mixes toward haze 0x3d4a68 at a deliberately lighter density 0.045 — World's 0.06 washed the alien pale at gameplay depths; `uDissolve` eats tail-up on death) — bell clone has `uSway` 0, tentacle clone 0.09 (shader writhes shafts, weighted to the tips via negative local y); both clones need every uniform update (uTime/uDissolve/uOpacity). Wisp trail, movement patterns fitted inside `X_BOUND` (weave recentered, hard clamp for all), state machine, hit sphere, `getHitCenter()`. Dies by dissolve + scale-collapse + violet fx burst + short-lived violet impact PointLight. `dispose()` disposes only owned materials — geometry is module-level shared statics.
  ...
  - `Hands.ts` — first-person two-handed alien energy pistol attached to the camera. Procedural gun: dark metal frame/slide/grip/trigger-guard (MeshStandardMaterial), teal HDR-boosted energy vents + core window + muzzle ring (bloom, no per-vent lights). Detail furniture sells the fidelity: three metal tones (matte polymer frame / blued slide / bright machined accents), cocking serrations, top rib, slide-stop, takedown pin, mag release, beavertail, grip side-panels with screws, mag baseplate with a teal power-cell seam, barrel cooling fins + energy coil, muzzle crown + near-black bore, teal front-sight dot. Both hands grip it (same finger/palm/arm builders, **dark leather gloves with knuckle pads, back-of-hand guard plates, and buckled wrist straps** — bare skin glowed green under the teal lights): right fist on the handle, left bracing under the barrel. The whole assembly is scaled 0.55 (and arms a further 0.6) so the pistol reads as a handgun in frame, not a screen-filling prop. `throwFireball()` triggers a 0.30s recoil kick + 0.09s teal-white muzzle-flash PointLight + additive glow decay. A persistent breathing teal PointLight sits at the frame (replaces the old warm orb light in the light budget).
  - `World.ts` — Three.js scene, camera (added to scene for hand children to render), pale-haze FogExp2 (see Art Direction), ACES tone mapping, PostFX integration (render + resize), near-black ground with baked emissive moonlight pool + moonlit path strip (path texture doubles as color map and emissiveMap; transparent for feathered edges), merged silhouette fence (one draw call), the hovering `Ship`, moonlight DirectionalLight from the moon position (8,13,-44) @ 1.0, ambient 0.55, forward fill directional 0.3, flickering warm lantern light near the player (2.2, range 20). Exposes `spawnDropHeight` (= `SHIP_BAY_Y`) so GameManager drops aliens from the bay.
  ...
  - `environment/Ship.ts` — the mothership hovering over the spawn zone at (0, 6.4, −15); aliens drop out of its glowing underbelly bay. Near-black lathe-saucer hull + command dome (all `fog: false` like the moon, so it stays crisp above the heavy playfield fog), violet HDR bay blob + torus lip + a rotating ring of running lights + a wide additive abduction beam falling to the ground (all bloom-bright violet, matching the aliens). `SHIP_BAY_Y` is exported so GameManager knows the drop height. `update(time)` bobs the hull, orbits the lights, pulses the bay/beam.
  - `environment/Sky.ts` — 3-stop gradient dome (horizon 0x3d4a68 → mid 0x1a2440 → zenith 0x0a0e22, BackSide ShaderMaterial), ~320 twinkling star Points (vertex-shader animated — twinkle drives brightness AND point size; ~12% are larger bright stars; cool #c8d8f4 tint), moon upper-right at (8,13,-44) with radial-gradient disc (#f2f6ff → #8ca8d0) + one smooth canvas-gradient halo sprite (layered flat discs band visibly), two hand-colored hill ridge layers (ShapeGeometry paper-cut silhouettes, farther = lighter). All `fog: false`.
  - `environment/Props.ts` — dead trees (recursive branching merged into one geometry), boulder clusters placed from a `boulderSpots` array (one large half-sunken tumbled dodecahedron + smaller satellites per spot — replaced cone "crystal shards" that just read as cones), patch-clustered grass with wind sway injected via `onBeforeCompile` (displacement ∝ height², `frustumCulled = false` since tips exceed static bounds) + rocks (each scatter one merged geometry) — all silhouette cutouts (grass has its own material for the wind uniform, rest share `SILHOUETTE_MAT`); drifting additive mist planes, occluding smoke clouds (normal alpha blending + renderOrder 3 so they genuinely hide aliens drifting behind them — a difficulty mechanic, not just decoration), violet spore-mote Points (fully vertex-shader animated, same color language as the alien rims), plus exported CanvasTexture makers for the ground's emissive moonlight pool (not tiled, maps 1:1) and the path (alpha-feathered ragged edges — its material needs `transparent: true`).
...
- Bloom sizing: `composer.setSize` multiplies by pixelRatio internally and overwrites pass sizes, so `bloomPass.setSize` must be re-called *after* it with `css × min(dpr, 1.5 mobile / 2 desktop)` — plain CSS size gave quarter-device-res glow on phones (blocky "old TV" smear)
